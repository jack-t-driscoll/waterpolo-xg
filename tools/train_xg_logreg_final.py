...
def build_design(df: pd.DataFrame, y: np.ndarray, folds: int):
    ...
    # Robust numeric coercion (avoid df.get(...) scalar trap)
    for c in base_num:
        if c in df.columns:
    ...
    # In-game priors (default to 0.0 when absent)
    prior_num = [
    ...
    # Booleans → ints (preserve your original map_bool helper)
    for b in ["is_man_up", "empty_net", "is_heave", "is_long_range"]:
        df[b] = map_bool(df.get(b, 0))

    # Categorical features (full-length Series, no scalar .get())
    for c in [
        ...
        if c in df.columns:
            ...
            df[c] = pd.Series("Unknown", index=df.index, name=c)

    # Collapsed distance bin + crosses for the model (same intent as your file)
    df["distance_bin_model"] = df["distance_bin"]
    df["dist_x_manup_model"] = "MU" + df["is_man_up"].astype(str) + "|" + df["distance_bin_model"].astype(str)
    df["dist_x_defbin_model"] = df["distance_bin_model"].astype(str) + "|" + df["defender_count_bin"].astype(str)

    # Group key (as in your file)
    if "game_id" not in df.columns:
        raise SystemExit("features_shots.csv missing 'game_id'")
    groups = df["game_id"].astype(str)

    # OOF opponent mean encoding (your helper)
